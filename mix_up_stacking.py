...
class MixUpStacking(object):
    def __init__(self):
        self.models = []
        self.n_models = 2
        self.n_mix = 1
        self.stacking_model = KernelRidge()
    ...
    def fit(self, train_data: pd.DataFrame):
        ...
        results = pd.DataFrame(index=train_data.index, columns=range(self.n_models))
        for i_model in range(self.n_models):
            results[i_model] = self.models[i_model].predict(X)
        self.stacking_model.fit(results, y)
    ...
    def predict(self, test_data: pd.DataFrame) -> pd.DataFrame:
        ...
        pred = post_precess(self.stacking_model.predict(results))
